...
const addCategoryOffer = async (req, res) => {
    try {
        ...
        // Update the category offer correctly
        await Category.updateOne({ _id: categoryId }, { $set: { categoryOffer: percentage } });

        for (const product of products) {
            product.productOffer = 0;
            product.salePrice = product.regularPrice;
            product.salePrice = product.regularPrice - (product.regularPrice * (percentage / 100)); // Apply offer
            await product.save();
        }

        console.log("Category updated:", categoryId, "Offer:", percentage);
